for n=1:Num
    ...
    p_i(n+1)=p_i(n)+H*(alpha(n,1)*u_i(n)*p_i(n)+beta1(n,1)*s_i(n)*p_i(n)+delta(n,1)*h_i(n)*p_i(n)-beta(n,1)*p_i(n)*s_i(n)-gamma(n,1)*p_i(n)*h_i(n)-omega*p_i(n)+lembda1(n,1)*w_i(n)*u_i(n)+lembda2(n,1)*c_i(n)*s_i(n)+lembda3(n,1)*g_i(n)*h_i(n));
    s_i(n+1)=s_i(n)+H*(alpha1(n,1)*u_i(n)*s_i(n)+beta(n,1)*p_i(n)*s_i(n)+delta1(n,1)*h_i(n)*s_i(n)-beta1(n,1)*s_i(n)*p_i(n)-gamma1(n,1)*s_i(n)*h_i(n)-omega*s_i(n)-lembda2(n,1)*c_i(n)*s_i(n));
    h_i(n+1)=h_i(n)+H*(gamma(n,1)*p_i(n)*h_i(n)+gamma1(n,1)*s_i(n)*h_i(n)-delta(n,1)*h_i(n)*p_i(n)-delta1(n,1)*h_i(n)*s_i(n)-omega*h_i(n)-lembda3(n,1)*g_i(n)*h_i(n));
